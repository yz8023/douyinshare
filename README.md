...
┌──────────────────────────────┐
│  内置解析通道（默认，无需部署）│
│  · BuiltInServer 127.0.0.1   │
│  · BuiltInParser 译写 data.php│
└───────────┬──────────────────┘
            │  携带登录 Cookie（内置登录页）
            ▼
        抖音开放接口 / 分享页
        （外部 PHP API 可选，部署后覆盖内置通道）
```

**默认使用 App 内置解析通道**：`BuiltInParser` 在进程内实现了 `server/data.php`
的解析核心（分享页 `_ROUTER_DATA` 提取、画质候选、原画质/最高画质、302 跟随），
通过 `BuiltInServer`（127.0.0.1 + 随机端口）以与服务器版兼容的接口形态提供服务，
客户端无需改动。想自建服务端的用户可部署 `server/` 并在设置里覆盖。

> ✅ **App 内置了解析服务器，安装即可用，无需自建服务端。**
> 解析、保存、画质切换默认走 App 内置的解析通道（进程内 127.0.0.1 HTTP 服务，
> 复用 App 内置登录的抖音 Cookie），打开即用。
> 外部服务器仍是**可选项**：想自建服务端的用户可按 [`server/README.md`](server/README.md)
> 部署并在「设置 → 服务器配置」里填写，填了之后自动覆盖内置通道。

## 给使用者：安装与配置

> 只想用这个 App、不想写代码的话，看这一节就够了。

**开箱即用：安装 APK 即可解析。** 解析默认走 App 内置通道，不依赖任何外部服务器；
内置通道支持单条解析 / 保存 / 画质切换，原画质与最高画质请在「设置 → 登录抖音账号」里
用 App 内置登录页登录一次（登录 cookie 保存在本机，仅用于解析）。

如果你自己部署了服务端，也可以按下面步骤切换到外部服务器（可选）。
...
3. **（可选）在 App 里配置外部服务器**：打开 App → **设置 → 服务器配置** → 填入解析接口地址、作者列表接口地址、API Token、HMAC 密钥 → 点 **测试连接** → 通过后点 **保存**。
   - 不填则继续使用内置通道；填了会覆盖内置通道。
...
- 它们只是**构建时的默认值**。App 默认使用内置解析通道；想用外部服务器的使用者在 App 里「设置 → 服务器配置」填写后会覆盖内置通道（存本机 SharedPreferences，优先级最高），所以**分发的 APK 可以只带占位地址**，不必泄露你的服务器。
