...
object ServerApiClient {
    ...
    private fun parseServerResponse(body: String, input: String, batchId: String?): ParseResult {
        ...
        val qualityList = runCatching {
            root.get("quality_list")?.takeIf { it.isJsonArray }?.asJsonArray?.mapNotNull { el ->
                ...
                val label = runCatching {
                    obj.get("label")?.takeIf { !it.isJsonNull }?.asString
                }.getOrNull() ?: return@mapNotNull null
                val ratio = runCatching {
                    obj.get("ratio")?.takeIf { !it.isJsonNull }?.asString
                }.getOrNull() ?: "default"
                val sizeBytes = runCatching {
                    obj.get("size_bytes")?.takeIf { !it.isJsonNull }?.asLong
                }.getOrNull()?.takeIf { it > 0 }
                val bitRate = runCatching {
                    obj.get("bit_rate")?.takeIf { !it.isJsonNull }?.asLong
                }.getOrNull() ?: 0L
                val isOriginal = runCatching {
                    obj.get("is_original")?.takeIf { !it.isJsonNull }?.asBoolean
                }.getOrNull() ?: false
                VideoQualityOption(
                    label = label,
                    ratio = ratio,
